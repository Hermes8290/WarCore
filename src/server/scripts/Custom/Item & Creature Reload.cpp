...
public:
	...
	std::vector<ChatCommand> GetCommands() const
	{
		static std::vector<ChatCommand> customCommandTable =
		{
			...
			{ "reload_creature_template", rbac::RBAC_PERM_COMMAND_RELOAD, true, &HandleReloadCreatureTemplate, "" },
			{ "reload_creature", rbac::RBAC_PERM_COMMAND_RELOAD, true, &HandleReloadCreature, "" },
			{ "reload_full_creature", rbac::RBAC_PERM_COMMAND_RELOAD, true, &HandleReloadFull_Creature, "" },
		};
	...
	static bool HandleReloadCreatureTemplate(ChatHandler * handler, const char * args)
	{
	...
	static bool HandleReloadCreature(ChatHandler * handler, const char * args)
	{
		bool creatureReloadEnabled = true;
		if (creatureReloadEnabled)
		{
			...
			//No need to add checks here because InitializeQueriesData checks if the config setting is enabled or disabled
			sObjectMgr->InitializeQueriesData(QUERY_DATA_CREATURES);
			handler->SendGlobalGMSysMessage("Creature has been reloaded");
		}
		else
			handler->SendGlobalGMSysMessage("The creature reload command is currently disabled");

		return true;
	}

	static bool HandleReloadFull_Creature(ChatHandler * handler, const char * args)
	{
		bool creatureReloadEnabled = true;
		if (creatureReloadEnabled)
		{
			TC_LOG_INFO("misc", "Reloading Full_Creature ...");
			...
			//No need to add checks here because InitializeQueriesData checks if the config setting is enabled or disabled
			sObjectMgr->InitializeQueriesData(QUERY_DATA_CREATURES);
			handler->SendGlobalGMSysMessage("Creature's has been reloaded");
		}
		else
			handler->SendGlobalGMSysMessage("The creature's reload command is currently disabled");
